...
function Company() {
  const [title] = useTypewriter({
    words: ['쉽고 빠르게 완성하는 나만의 이력서'],
    loop: 1, // 무한 루프 여부
    typeSpeed: 100,
  });
  const {ref:main2Ref, inView: main2InView} = useInView({
    triggerOnce: true,
    threshold: 0.1,
  });
  const {ref:main3Ref, inView: main3InView} = useInView({
    triggerOnce: true,
    threshold: 0.1,
  });
  console.log(main2InView); // 상태 확인용 로그 추가
  return (
    <div className='main'>
      <div className='main-1'>
        ...
        <div className={`main-2 ${main2InView ? 'fade-in' : ''}`} ref={main2Ref}>
        <div className='content'>
        ...
        <div className={`main-3 ${main3InView ? 'fade-in' : ''}`} ref={main3Ref}>
        <div className='content'>
