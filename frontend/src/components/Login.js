...
function Login(props) {
    ...
    // user data가져오기
    const getuserinfo = () =>{
        fetch("http://192.168.0.114:5000/api/accesstoken", {
    ...
    // 로그아웃 요청
    const logout =() =>{
        fetch("http://192.168.0.114:5000/api/auth/sign-out", {
            method: "POST",
            credentials: 'include',
        })
        .then(response=>response.json())
    ...
    // 회원탈퇴 요청
    const user_delete =() =>{
        fetch("http://192.168.0.114:5000/delete", {
            method: "POST",
            headers:{
    ...
    const savelogin = async (e) =>{
        ...
        try{
                const response = await fetch('http://192.168.0.114:5000/api/auth/sign-in',{
                    method: 'POST',
    ...
    const accession = async (e) =>{
        ...
        try{
            const response = await fetch('http://192.168.0.114:5000/accession',{
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
    ...
    const id_check = async (e) =>{
        e.preventDefault();
        try{
            const response = await fetch("http://192.168.0.114:5000/id_check", {
                method: "POST",
                headers: {
                    'Content-Type': 'application/json'
                },
    ...
    useEffect(() => {
        // user.user 상태가 변경될 때만 로그인 상태 업데이트
        if (user.user) {
            setislogin(true);
        } else {
            setislogin(false);
        }
    }, [user.user]); // user.user가 변경될 때마다 실행
